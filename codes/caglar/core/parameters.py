...
from utils.inits import (sample_weights_orth,
...
class BiasInitializer(Initializer):
    ...
    def __call__(self, ndim, init_bias_val=None):
        if self.init_method == BiasInitMethods.Random:
            bias = self.rng.uniform(-self.scale, self.scale, (ndim))
...
class Parameters(object):
    """
        A class for representing the parameters of a model. This is
    basically a large dictionary, but has some utility functions
    for managing the properties related to the parameters.
    """
    def __init__(self):
    ...
    def __merge_with_other(self, other):
        other_dict = other.__dict__['params']
        current_dict = self.__dict__['params']
        new_p = Parameters()
    ...
    def save(self, filename):
        params = self.__dict__['params']
        pkl.dump({p.name:p.get_value() for p in params.values()}, open(filename, 'wb'), 2)

    def update(self, new_val):
        if isinstance(new_val, Parameters):
            new_dict = new_val.__dict__['params']
        else:
    ...
    def set_values(self, other):
        ...
        [params[k].set_value(v.get_value()) if hasattr(v, 'get_value') else params[k].set_value(v) \
    ...
    def print_param_norms(self):
        ...
        for k, v in params.iteritems():
            print "param name: %s, param norm: %.2f " % (k, np.sqrt(((v.get_value())**2).sum()))
